<template>
  ...
  >
    <v-container style="padding-left: 0px;" pt-4>
      <v-form ref="schedules" onSubmit="return false;" lazy-validation>
        <v-layout
          wrap
          row
            ...
            <v-layout wrap row>
              <v-flex xs12 sm12 lg12 md12 xl12>
                <v-layout wrap row>
                  ...
                  ></div>
                  <v-flex :class="$vuetify.breakpoint.mdAndUp?'md7 lg3 xl3':'xs11 sm11'">
                    <v-select
                      class="wrapperSelectSchedule"
                      ...
                      @change="disableItem($event,schedule)"
                    ></v-select>
                  </v-flex>
                  ...
                  <v-flex :class="$vuetify.breakpoint.mdAndUp?'md10 lg5 xl6':'xs12 sm12'">
                    <v-layout wrap row>
                      ...
                      <v-flex
                        xs12
                        sm12
                        style="text-align: end;height: 34px;"
                        v-if="!$vuetify.breakpoint.mdAndUp"
                      >
                        <div style="line-height: 65px;">
                          <v-icon
                            :style="'cursor:pointer;    margin-right: -8px;'"
                            :title="$t('message.vBusinessAddNewSchedule')"
                            @click="addNewSubSchedule(index)"
                          >mdi mdi-calendar-plus</v-icon>
                        </div>
                      </v-flex>
                    </v-layout>
                  </v-flex>
                  ...
                  <v-flex v-if="$vuetify.breakpoint.mdAndUp" md3 lg3 xl3></v-flex>
                  <div
                    v-if="$vuetify.breakpoint.mdAndUp"
                    :class="$vuetify.breakpoint.mdAndUp?'switchStatus pl-3':' flex xs12 sm12'"
                  >
                    <v-layout wrap row>
                      <v-flex sm3 xs3 lg6 xl6></v-flex>
                      <v-flex sm4 xs4 lg4 xl4></v-flex>
                    </v-layout>
                  </div>
                  <v-flex xs12 sm5 lg5 md6 xl5></v-flex>
                </v-layout>
              </v-flex>
              <v-flex
                xs12
                ...
                md12
                xl12
                v-for="(subSchedule,i) in schedule.subSchedules"
                :key="i"
              >
                <v-layout>
                   <v-flex v-if="$vuetify.breakpoint.lgAndUp" :class="$vuetify.breakpoint.lgAndUp?'md3 lg3 xl3':'xs11 sm11'">
                   
                  </v-flex>
                    <div
                    v-if="$vuetify.breakpoint.lgAndUp"
                    :class="$vuetify.breakpoint.lgAndUp?'switchStatus pl-3':' flex xs12 sm12'"
                  >
                    <v-layout wrap row>
                      <v-flex sm3 xs3 lg6 xl6>
                      </v-flex>
                      <v-flex sm4 xs4 lg4>
                      </v-flex>
                    </v-layout>
                  </div>
                  <v-flex xs12 sm12 lg5 md10 xl6>
                    <v-flex xs12 sm12 lg12 md12 xl12>
                      <v-flex :class="$vuetify.breakpoint.mdAndUp?'md12 lg12 xl12':'xs12 sm12'">
                        <v-layout wrap row>
                          <v-flex sm12 xs12 lg5 md5 xl5>
                            <v-menu
                              :ref="'menuSubSchedule'+index+''+i"
                              :close-on-content-click="false"
                              v-model="subSchedule.menuO"
                              :nudge-right="40"
                              :return-value.sync="subSchedule.open"
                              lazy
                              transition="scale-transition"
                              offset-y
                              full-width
                            ...
                            >
                              <v-text-field
                                :color="currentBussinesClub.theme.primary"
                                slot="activator"
                                v-model="subSchedule.open"
                                :label="$t('message.openAt')"
                                readonly
                              ></v-text-field>
                              <v-time-picker
                                :color="currentBussinesClub.theme.primary"
                                v-if="subSchedule.menuO"
                                v-model="subSchedule.open"
                                full-width
                                @change="saveHour('menuSubSchedule'+index+''+i,subSchedule.open)"
                              ></v-time-picker>
                            </v-menu>
                          </v-flex>
                          <div v-if="$vuetify.breakpoint.mdAndUp">
                          ...
                          <v-flex sm12 xs12 lg5 md5 xl5>
                            <v-menu
                              :ref="'menuSubScheduleClose'+index+''+i"
                              :close-on-content-click="false"
                              v-model="subSchedule.menuC"
                              :nudge-right="40"
                              :return-value.sync="subSchedule.close"
                              lazy
                              transition="scale-transition"
                              offset-y
                              full-width
                            ...
                            >
                              <v-text-field
                                :color="currentBussinesClub.theme.primary"
                                slot="activator"
                                v-model="subSchedule.close"
                                :label="$t('message.closeAt')"
                                readonly
                              ></v-text-field>
                              <v-time-picker
                                :color="currentBussinesClub.theme.primary"
                                v-if="subSchedule.menuC"
                                v-model="subSchedule.close"
                                full-width
                                @change="saveHour('menuSubScheduleClose'+index+''+i,subSchedule.close)"
                              ></v-time-picker>
                            </v-menu>
                          </v-flex>

                          <v-flex style="text-align: end;" v-if="!$vuetify.breakpoint.mdAndUp">
                            <div style="line-height: 65px;margin-left: 5px;">
                              <v-icon
                                :style="'cursor:pointer;  margin-right: -9px;'"
                                :title="$t('message.vBusinessAddNewSchedule')"
                                @click="deleteSubSchedule(index,i)"
                              >mdi mdi-delete</v-icon>
                            </div>
                          </v-flex>
                        </v-layout>
...
export default {
  ...
  methods: {
    deleteSubSchedule(index, subIndex) {
      this.schedules[index].subSchedules.splice(subIndex, 1);
    },
    addNewSubSchedule(index) {
      if (this.schedules[index].subSchedules.length < 3) {
        let newSubSchedule = {
          open: "",
          close: "",
          menuO: false,
        ...
        this.schedules[index].subSchedules.push(newSubSchedule);
      }
    },
    saveSchedule() {
      this.open = false;
