						...
						<v-flex v-bind:class="{'md-limit-sm':$vuetify.breakpoint.smAndDown, 'md-limit-md':$vuetify.breakpoint.mdAndUp }">
							<v-card color="transparent" flat>
								...
								<v-card-title primary-title>
									<div>
										<h3 class="headline mb-2 white--text">{{$t('message.thanksMemberJoin')}}</h3>
										<div class="mb-3 white--text">{{$t('message.thanksMemberJoinDescription')}}</div>
...
export default {
	...
	mounted() {
		...
		let referral = url.searchParams.get("invite");
		if (referral) {
			this.$store.dispatch("agregateMember", referral).then(res => {
